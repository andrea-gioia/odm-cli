...
@Command(name = "upload", description = "Upload a Data Product Version from a git repository", mixinStandardHelpOptions = true, version = "odm-cli registry upload 1.0.0")
public class RegistryUploadCommand implements Runnable {

    @Option(names = "--type", description = "Type of the entity to list (dp|dpv)", defaultValue = "dpv", required = false)
    String entityType;
    ...
    public void uploadDataProductVerion() {
        ...
        try {
            ResponseEntity<String> dataProductResponseEntity = registryCommands.getRegistryClient()
                    .uploadDataProductVersion(
