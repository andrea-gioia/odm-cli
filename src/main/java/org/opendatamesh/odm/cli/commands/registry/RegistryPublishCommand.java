...
public class RegistryPublishCommand implements Runnable {
    ...
    String entityDescriptionFilePath;

    @Option(names = "--type", description = "Type of the entity to list (dp|dpv)", defaultValue = "dp", required = false)
    String entityType;

    @Option(names = "--dp-id", description = "Id of data product. Applicable only if type is equal to dpv", required = false)
    ...
    public void publishDataProduct() {
        String dp;
        try {
            dp = CliFileUtils.readFileFromPath(entityDescriptionFilePath);
        } catch (IOException e) {
            System.out.println(
                    "Impossible to read file [" + entityDescriptionFilePath
                            + "]. Check if the file exists and retry");
            return;
        }
        try {
            ResponseEntity<DataProductResource> dataProductResponseEntity = registryCommands
                    .getRegistryClient().postDataProduct(dp);
    ...
    public void publishDataProductVerion() {
        String dpv;
        try {
            dpv = CliFileUtils.readFileFromPath(entityDescriptionFilePath);
        } catch (IOException e) {
            System.out.println(
                    "Impossible to read file [" + entityDescriptionFilePath +
                            "]. Check if the file exists and retry");
        ...
        System.out.println("Publishing a new version of product [" + dataProductId + "]: \n" + dpv);

        try {
            ResponseEntity<DataProductVersionDPDS> dataProductResponseEntity = registryCommands
                    .getRegistryClient().postDataProductVersion(dataProductId, dpv);
            if (dataProductResponseEntity.getStatusCode().equals(HttpStatus.CREATED)) {
